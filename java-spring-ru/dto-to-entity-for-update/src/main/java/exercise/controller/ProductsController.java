...
public class ProductsController {
    ...
    public ProductDTO update(@RequestBody ProductUpdateDTO productData, @PathVariable long id) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not dound"));
        toEntity(productData, product);
        productRepository.save(product);
        var res = toDTO(product);
        return res;
    }

    // for update. Здесь мы понимаем, что для реализации апдейта нужно перегрузить метод toEntity на реализацию ProductUpdateDTO
    public Product toEntity(ProductUpdateDTO data, Product p) {
